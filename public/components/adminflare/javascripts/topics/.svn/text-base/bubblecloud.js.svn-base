...
Bubbles = function(top,left) {
    ...
    node = null;
    label = null;
    var topDelta = top; var leftDelta = left;
    margin = {
        top: 0,
        right: 0,
        bottom: 0,
    ...
    tick = function(e) {
        ...
        return label.style("left", function(d) {
            //console.log("left: " + d.x + " dx:"+ d.dx + " - calc: " + ((margin.left + d.x) - d.dx / 2));
            return ((margin.left + d.x) - d.dx / 2) + "px";
        }).style("top", function(d) {
                //console.log("top: " + d.y + " dy:"+ d.dy + " - calc: " + ((margin.top + d.y) - d.dy / 2));
    ...
    chart = function(selection) {
        return selection.each(function(rawData) {
            ...
            console.log("selectAll : " + d3.select(this).selectAll("svg").length);
            svg = d3.select(this).selectAll("svg").data([data]);
            svgEnter = svg.enter().append("svg").attr("id", "svg1");
...
$( function handler() {
        var display,display2, key, plot, plot2, text;
        plot = Bubbles();
        plot2 = Bubbles();
        display = function(data) {
            //console.log(data);
            return plotData("#container2", data, plot);
        };

        display2 = function(data) {
            //console.log(data);
            return plotData("#container1", data, plot2);
        };
        ...
        d3.csv("/data/bubblecloud/topics.csv", display);

        var kickoff = function() {
            d3.csv("/data/bubblecloud/keywords.csv", display2);
        }
